...
**実測の構成比:** Week2は nb_* 408 / jp_* 11 / own_* 9 だったが、
Week3は **own_* 79** / nb_* 16 / jp_* 8。自作素材が主役に移っている。
初期に定めた「標準プール12ワード」(city night ほか)はWeek3で104カット中1カットのみ。
**新規台本ではこの語彙表から選ぶこと。検索ワードのリストからは選ばない。**

### 新しくPexelsから取ったら、検索ワードをここに記録する

```bash
.venv/bin/python tools/fetch_bg_pexels.py "検索ワード" ファイル名.mp4
```

取得したら、下の語彙表に追記するとき **クリップ名の後ろに `(検索ワード)` を書き添える。**
これがないと、同じ系統の絵を後から足したいときに元のクエリが分からなくなる。

> ⚠ 既存の `jp_*`(25本)と `nb_*`(30本)は**検索ワードが未記録**。
> ファイル名から意図は読めるが、正確なクエリは残っていない。
> 判明している範囲: `jp_*` は「渋谷/新宿ネオン・電車・スカイライン・日本のカフェ作業・和食」
> 系のワードで取得(2026/7/28のコミットより)。再取得が必要になったら作り直す前提で考えること。
