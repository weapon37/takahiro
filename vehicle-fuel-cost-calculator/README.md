...
- `init_sheet.py` — 月次タブ(運行表+ガソリン代按分)のテンプレートをスプレッドシートに作成。1年分まとめて作成も可能
...
対象月のタブ(運行表+ガソリン代按分のテンプレート)を作成します。

```bash
python init_sheet.py --month 2026.8                 # 1か月分
python init_sheet.py --month 2026.9 2026.10         # 複数まとめて
python init_sheet.py --month 2026.9 --count 12      # 2026.9〜2027.8を一括作成
```

- タブ名は「YYYY.M」形式(例: `2026.8`)。年またぎ(2026.12→2027.1)も `--count` で正しく処理されます
- 既存の同名タブがある場合は**中身を上書き**します。実データが入っているタブに対して実行しないよう注意してください
- 複数月を指定しても、書き込みは一括リクエストにまとめられるためAPI呼び出し回数は増えません

### 3-0b. (任意)運行表の画像をOCRで読み取る
